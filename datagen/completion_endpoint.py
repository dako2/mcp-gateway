...
def get_args():
    ...
    parser.add_argument('--engine', default="vllm", type=str, choices=["vllm", "vllm_api", "hf", "together_api", "openai", "openrouter_api", "xai"])
...
def generate_and_update(dataset, checkpoint_file, llm=None, params=None, tokenizer=None):
    ...
    for i in tqdm(range(num_batches)):
        ...
        elif args.engine in ("openai", "openrouter_api", "xai"):
...
def main():
    ...
    if args.engine == "together_api":
        ...
        params = None
        tokenizer = None
    elif args.engine == "xai":
        print("Start xAI Grok engine...")
        xai_api_key = args.xai_api_key if args.xai_api_key else os.getenv("XAI_API_KEY")
        if not xai_api_key:
            raise ValueError("xAI API Key not provided. Please set XAI_API_KEY environment variable or provide --xai_api_key argument.")
